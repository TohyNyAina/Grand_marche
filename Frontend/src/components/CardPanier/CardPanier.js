              ...
              <div className="pannier-container-1">
                <div className="pannier-section-1">
                  <div className="section-1">
                    ...
                    <div>{panier_datas.length > 0 ? <p>Il n'y a rien dans votre panier</p> : null}</div>
                    ...
                    <div className="pannier-produit-section">
                      {panier_datas.map((panier_data) => (
                        <div key={panier_data.name} className="w-full flex flex-row border-b py-2 ">
                          ...
                          <div className="">
                            <div className="mt-3">
                              <button onClick={() => dispatch(addToPanier(panier_data))} className="">
                                +1
                              </button>
                              <span className="mt-3 ml-3 bg-gray-300 p-2 ">{panier_data.nombre}</span>
                              <button onClick={() => dispatch(decrementPanier(panier_data))} className="ml-3">
                                -1
                              </button>
                            </div>
                            <br />
                            <button
                              onClick={() => dispatch(removeToPanier(panier_data._id))}
                              className="bg-blue-900 w-28 h-12 text-white rounded-lg hover:bg-blue-800"
                            >
                              Retirer
                            </button>
                            <hr />
                          ...
                          <div className="prix">
                            {panier_datas.map((produit_data) => (
                              <div className="flex justify-between px-2">
                                <p>
                                  {produit_data.nombre} {produit_data.name}
                                </p>
                          ...
                          <div className="panier-total">
                            ...
                            <div className="total-container">
                              ...
                              <div className="total-prix">
                                <span>
                                  {panier_datas.length > 0
                                    ? panier_datas.map((el) => el.prix * el.nombre).reduce((prev, cur) => prev + cur)
                                    : 0}{" "}
                                  MGA
                                </span>
                              </div>
